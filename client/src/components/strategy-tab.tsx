...
import { Printer, TrendingDown, Zap, Droplets, Wind, Lightbulb, Flame, DollarSign } from "lucide-react";
...
export default function StrategyTab({ project }: StrategyTabProps) {
  ...
  // Empreinte économique — editable costs per unit (hooks must be before early return)
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="mx-8 mb-6">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-1 h-6 rounded-full" style={{ backgroundColor: "#16a34a" }} />
                    <h2
                      className="text-base font-bold uppercase tracking-widest"
                      style={{ fontFamily: "'Inter', sans-serif", color: "#1e3a5f" }}
                    >
                      Empreinte économique
                    </h2>
                    <div className="flex-1 h-px" style={{ backgroundColor: "#1e3a5f22" }} />
                  ...
                  {/* Cost table */}
                  <div className="rounded-xl overflow-hidden border" style={{ borderColor: "#1e3a5f22" }}>
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ backgroundColor: "#1e3a5f" }}>
                          <th className="px-4 py-3 text-left text-white font-semibold text-xs uppercase tracking-wider">Mesure</th>
                          <th className="px-4 py-3 text-center text-white font-semibold text-xs uppercase tracking-wider">Unités</th>
                          <th className="px-4 py-3 text-right text-white font-semibold text-xs uppercase tracking-wider">Coût / unité</th>
                          <th className="px-4 py-3 text-right text-white font-semibold text-xs uppercase tracking-wider">Total</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y" style={{ borderColor: "#1e3a5f11" }}>

                        {showHeatingStrategy && (
                          <tr className="hover:bg-slate-50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Zap className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#1e3a5f" }} />
                                <div>
                                  <p className="font-medium text-slate-700">Thermopompes</p>
                                  <p className="text-xs text-slate-400">Innovair QHW12H2UZRA / QOS12H2BM5A</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-center text-slate-600 font-medium">{nbThermo}</td>
                            <td className="px-4 py-3 text-right">
                              <input
                                type="number"
                                value={coutThermo}
                                onChange={(e) => setCoutThermo(Number(e.target.value))}
                                className="w-24 text-right text-slate-700 font-medium bg-blue-50 border border-blue-200 rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
                              />
                        ...
                        {showHeatPumpWaterHeaterStrategy && (
                          <tr className="hover:bg-slate-50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Droplets className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#1e3a5f" }} />
                                <div>
                                  <p className="font-medium text-slate-700">Chauffe-eau thermopompe</p>
                                  <p className="text-xs text-slate-400">Remplacement chauffe-eau existant</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-center text-slate-600 font-medium">{nbUnits}</td>
                            <td className="px-4 py-3 text-right">
                              <input
                                type="number"
                        ...
                        {showVrcStrategy && (
                          <tr className="hover:bg-slate-50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Wind className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#1e3a5f" }} />
                                <div>
                                  <p className="font-medium text-slate-700">VRC</p>
                                  <p className="text-xs text-slate-400">Ventilateur récupérateur de chaleur</p>
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-center text-slate-600 font-medium">{nbUnits}</td>
                        ...
                        {showLedStrategy && (
                          <tr className="hover:bg-slate-50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <Lightbulb className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#1e3a5f" }} />
                                <div>
                                  <p className="font-medium text-slate-700">Éclairage DEL</p>
                                  <p className="text-xs text-slate-400">Remplacement luminaires existants</p>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3 text-right font-bold" style={{ color: "#1e3a5f" }}>
                              {totalLed > 0 ? `${totalLed.toLocaleString("fr-CA")} $` : <span className="text-slate-400">—</span>}
                            </td>
                          </tr>
                        )}
                      </tbody>
                      ...
                      <tfoot>
                        {/* Total brut */}
                        <tr style={{ backgroundColor: "#f8fafc", borderTop: "2px solid #1e3a5f22" }}>
                          <td colSpan={3} className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">
                          ...
                          <>
                            <tr className="bg-green-50">
                              <td className="px-4 py-2.5" colSpan={2}>
                                <p className="text-xs font-medium text-green-700">Possibilité de subvention — Thermopompes</p>
                                <p className="text-xs text-green-600 mt-0.5">{nbThermo} unité{nbThermo > 1 ? "s" : ""} × {SUBVENTION_THERMO.toLocaleString("fr-CA")} $/unité</p>
                              </td>
                              <td className="px-4 py-2.5 text-right text-xs text-green-600">Programme Hydro-Québec</td>
                              <td className="px-4 py-2.5 text-right font-bold text-green-700">
                                − {totalSubvention.toLocaleString("fr-CA")} $
                              </td>
                            </tr>
                            <tr style={{ backgroundColor: "#1e3a5f" }}>
                              <td colSpan={3} className="px-4 py-3 text-right text-white font-bold text-xs uppercase tracking-wider">
                  ...
                  <p className="text-xs text-slate-400 italic mt-3">
                    * Les montants sont des estimations à titre indicatif afin de planifier les travaux. Un devis détaillé doit être obtenu auprès des entrepreneurs qualifiés.
                  </p>
                </div>
