...
export default function ReportTab({
  ...
  const selectedHeatPump: HeatPump | undefined =
    ...
    heatPumpsData?.find(hp => hp.type === "heatpump" && hp.isDefault) ??
    heatPumpsData?.filter(hp => hp.type === "heatpump")[0]);
  const selectedWaterHeater: HeatPump | undefined =
  ...
  // ── Section Preuves (partagée avec Recommandations) ─────────────────────────
  ...
  const [preuvesSections, setPreuvesSections] = useState<PreuveSection[]>(
    ((project as any).annexPreuvesSections as PreuveSection[]) ?? []
  );
  ...
  async function handlePreuvesUpload(files: FileList | null) {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/projects", project.id] });
    } catch { toast({ title: "Échec de l'upload", variant: "destructive" }); }
  ...
  async function addPreuvesSection() {
    try {
      const res = await fetch(`/api/projects/${project.id}/preuves-section/add`, { method: "POST" });
      if (!res.ok) throw new Error();
      const updated = await res.json();
      setPreuvesSections(((updated as any).annexPreuvesSections as PreuveSection[]) ?? []);
      queryClient.invalidateQueries({ queryKey: ["/api/projects", project.id] });
    } catch { toast({ title: "Erreur lors de l'ajout", variant: "destructive" }); }
  }

  async function uploadToSection(sectionId: string, files: FileList | null) {
    if (!files || files.length === 0) return;
    ...
    try {
      ...
      for (const f of Array.from(files)) {
        const fd = new FormData();
        fd.append("file", f);
        const res = await fetch(`/api/projects/${project.id}/preuves-section/${sectionId}/upload`, { method: "POST", body: fd });
        if (!res.ok) throw new Error("Échec upload");
        lastProject = await res.json();
      }
      if (lastProject) setPreuvesSections(((lastProject as any).annexPreuvesSections as PreuveSection[]) ?? []);
      queryClient.invalidateQueries({ queryKey: ["/api/projects", project.id] });
    } catch { toast({ title: "Échec de l'upload", variant: "destructive" }); }
    finally {
      setUploadingSectionId(null);
      const ref = sectionInputRefs.current[sectionId];
      if (ref) ref.value = "";
    }
  }
  ...
  async function updateSectionTitle(sectionId: string, title: string) {
    try {
      await fetch(`/api/projects/${project.id}/preuves-section/${sectionId}/title`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteFromSection(sectionId: string, url: string) {
    try {
      const res = await fetch(`/api/projects/${project.id}/preuves-section/${sectionId}/image`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url }),
      });
  ...
  async function deleteSection(sectionId: string) {
    try {
      const res = await fetch(`/api/projects/${project.id}/preuves-section/${sectionId}`, { method: "DELETE" });
      if (!res.ok) return;
      const updated = await res.json();
      setPreuvesSections(((updated as any).annexPreuvesSections as PreuveSection[]) ?? []);
    } catch {}
  ...
  const showVrcStrategy = hasVrcInstallation(post) && !hasVrcInstallation(pre) && project.buildingType !== "new";
                ...
                {showHeatPumpWaterHeaterStrategy && (
                  ...
                  >
                    ...
                    <p>
                      Installation de{" "}
                      <span className="font-semibold">
                        {(project.nbChauffeEauThermo ?? numUnits) > 0 ? (project.nbChauffeEauThermo ?? numUnits) : "—"}
                      </span>{" "}
                  ...
                  <div className="mt-12" data-testid="strategy-basement-insulation">
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 items-end print:hidden" data-testid="insul-fields">
                      <div>
                        ...
                        <input
                          data-testid="input-insul-rvalue"
                          type="number"
                          defaultValue={project.basementInsulationRValue || ""}
                          onBlur={(e) => insulMutation.mutate({ basementInsulationRValue: e.target.value })}
                    ...
                    <>
                        ...
                        <>
                          ...
                          >
                            ...
                            {selectedHeatPump && ((selectedHeatPump.images as string[])?.length > 0 || (selectedHeatPump.specPages as string[])?.length > 0) ? (
                              ...
                              <>
                                <div className="print:break-after-page">
                                  <AnnexImageUpload key="annex-thermopompes" projectId={project.id} annexType="thermopompes" label="Thermopompes – photo" currentImage={annexImages.thermopompes} defaultImageUrl={tclPhotoPath} />
                                </div>
                      ...
                      {showHeatPumpWaterHeaterStrategy && (
                        <div id="annex-chauffe-eau-thermopompe" className="print:break-before-page">
                          <h3 className="text-sm font-semibold mb-2">
                            {annexNum++}. {selectedWaterHeater ? `${selectedWaterHeater.name}${selectedWaterHeater.model ? ` ${selectedWaterHeater.model}` : ""}` : "Chauffe-eaux Thermopompe Rheem PROPH40 T2 RH375-30"}
                          </h3>
                          ...
                          {selectedWaterHeater && ((selectedWaterHeater.images as string[])?.length > 0 || (selectedWaterHeater.specPages as string[])?.length > 0) ? (
                            ...
                            <>
                              <div className="print:break-inside-avoid mb-4">
                                <img src={rheemPage1Path} alt="Chauffe-eau hybride Rheem ProTerra — page 1" className="w-full rounded border" />
                              </div>
                      ...
                      <div className="print:break-before-page mt-6">
                        {exportMode ? (
                          <>
                            {(preuvesTitle || preuvesImages.length > 0) && (
                              <>
                                ...
                                <div className="space-y-4">
                                  {preuvesImages.map((url, i, arr) => (
                                    <div key={i} className={i < arr.length - 1 ? "print:break-after-page" : ""}>
                            ...
                            {preuvesSections.map((section) => (
                              (section.title || section.images.length > 0) ? (
                                <div key={section.id} className="mt-6">
                                  <h3 className="text-sm font-semibold mb-2">{annexNum++}. {section.title || "Preuves"}</h3>
                                  <div className="space-y-4">
                                    {section.images.map((url, i, arr) => (
                                      <div key={i} className={i < arr.length - 1 ? "print:break-after-page" : ""}>
                                        <img src={url} alt={`Preuve ${i + 1}`} className="w-full rounded border" />
                                      </div>
                                    ))}
                          ...
                          <>
                            ...
                            {preuvesSections.map((section) => (
                              <div key={section.id} className="mb-4 rounded-lg border border-dashed border-slate-200 p-3 bg-slate-50/50">
                                <div className="flex items-center gap-2 mb-3">
                                  <span className="text-sm font-semibold shrink-0">{annexNum++}.</span>
                                  <Input
                                  ...
                                  <label className="cursor-pointer shrink-0">
                                    <input
                                      ref={el => { sectionInputRefs.current[section.id] = el; }}
                                      type="file"
                                      accept="image/*,.pdf,application/pdf"
                                      multiple
                                      className="hidden"
                                      onChange={e => uploadToSection(section.id, e.target.files)}
                                    />
                                    <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors whitespace-nowrap">
                                      {uploadingSectionId === section.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <><Plus className="w-3.5 h-3.5" /> Ajouter</>}
                                    </span>
                                  </label>
                                  <button
                                    type="button"
                                ...
                                {section.images.length === 0 ? (
                                  <p className="text-xs text-slate-400 italic">Aucune image — ajoutez des photos ou preuves.</p>
                                ) : (
                                  <div className="grid grid-cols-2 gap-3">
                                    {section.images.map((url, i) => (
                                      <div key={i} className="relative group rounded-lg overflow-hidden border border-slate-200 bg-white">
                                        <img src={url} alt={`Preuve ${i + 1}`} className="w-full object-cover" />
                                        <button
                                          onClick={() => deleteFromSection(section.id, url)}
                                          className="absolute top-1 right-1 p-1 rounded-full bg-red-500 text-white opacity-0 group-hover:opacity-100 transition-opacity"
                                        >
                                          <X className="w-3 h-3" />
                                        </button>
