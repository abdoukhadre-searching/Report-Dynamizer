...
function getProjectStatus(p: Project): { label: string; color: string; bg: string; border: string } {
  if (p.preReportData && p.postReportData) return { label: "Qualifié", color: "#16a34a", bg: "#f0fdf4", border: "#bbf7d0" };
  if (p.preReportData) return { label: "En cours", color: "#d97706", bg: "#fffbeb", border: "#fde68a" };
...
function NewProjectDialog({
  ...
  onCreate: (type: "existing" | "new") => void; isPending: boolean;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) { onClose(); setSelected(null); } }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base font-bold text-slate-900">Nouveau projet — Type de bâtiment</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-4 py-2">
          <button
            type="button"
            onClick={() => setSelected("existing")}
            className={`flex flex-col items-center gap-3 rounded-xl border-2 p-5 transition-all cursor-pointer ${selected === "existing" ? "border-[#1e3a5f] bg-[#1e3a5f]/5" : "border-border hover:border-[#1e3a5f]/40 hover:bg-slate-50"}`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-3 rounded-xl border-2 p-5 transition-all cursor-pointer ${selected === "new" ? "border-[#16a34a] bg-[#16a34a]/5" : "border-border hover:border-[#16a34a]/40 hover:bg-slate-50"}`}
          >
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="ghost" size="sm" onClick={() => { onClose(); setSelected(null); }}>Annuler</Button>
          <Button
            size="sm"
...
export default function RapportsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (buildingType: "existing" | "new") => {
      const res = await apiRequest("POST", "/api/projects", { name: "Nouveau projet", status: "draft", buildingType });
      return res.json();
    },
      ...
      <header className="sticky top-0 z-40 border-b" style={{ backgroundColor: "rgba(255,255,255,0.96)", backdropFilter: "blur(8px)", borderColor: "#e8edf4" }}>
        <div className="max-w-3xl mx-auto px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" className="gap-1.5 font-semibold" style={{ backgroundColor: "#1e3a5f" }} onClick={() => setShowNewDialog(true)} data-testid="button-new-project">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="grid sm:grid-cols-2 gap-5 mb-10">
          ...
          >
            ...
            <div className="pl-7 pr-6 pt-6 pb-6">
              ...
              <p className="text-sm text-slate-500 leading-relaxed mb-5">Analyse avant / après travaux pour immeubles déjà construits.</p>
          ...
          >
            ...
            <div className="pl-7 pr-6 pt-6 pb-6">
              ...
              <p className="text-sm text-slate-500 leading-relaxed mb-5">Qualification selon le bâtiment de référence CNEB 2017.</p>
              <div className="flex items-center justify-between mt-auto">
                ...
                return (
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: isNew ? "#f0fdf4" : "#eef2f8" }}>
                      {isNew ? <HardHat className="w-4 h-4" style={{ color: "#16a34a" }} /> : <Building2 className="w-4 h-4" style={{ color: "#1e3a5f" }} />}
                    </div>
        ...
        {allProjects.length === 0 && (
          <div className="text-center py-16">
            ...
            <h2 className="text-base font-semibold text-slate-700 mb-2">Aucun projet HOT2000</h2>
            <p className="text-sm text-slate-400 mb-6">Créez votre premier projet pour analyser des rapports HOT2000.</p>
            <Button className="gap-2 font-semibold" style={{ backgroundColor: "#1e3a5f" }} onClick={() => setShowNewDialog(true)}>
